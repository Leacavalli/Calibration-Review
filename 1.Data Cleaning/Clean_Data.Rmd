...
Covidence_raw_data_1cal <- Covidence_raw_data |>
  ...
  filter(! Covidence.. %in% c(2507, 2756, 1782 ,2742)) 
```
...
mutate(Name.of.calibration.algorithm = ifelse(Name.of.calibration.algorithm== "Other: Not reported ", "Not reported", Name.of.calibration.algorithm))|>
  ...
  mutate(Model.type = ifelse(Model.type== "Compartmental ", "Compartmental", Model.type))|> 
    ...
    mutate(Nature.of.calibration.results = ifelse(grepl("Point",Nature.of.calibration.results), "Point estimate (single parameter / parameter set)", Nature.of.calibration.results))|> 
    mutate(Scientific.problem.being.solved = ifelse(Scientific.problem.being.solved =="Evaluate interventions, Assess impact of model assumptions", "Evaluate interventions; Assess impact of model assumptions", Scientific.problem.being.solved))|> 
    mutate(Scientific.problem.being.solved = ifelse(Scientific.problem.being.solved == "Assess impact of model assumptions; Understand disease mechanisms (e.g., through inference on a parameter)", "Assess impact of model assumptions; Understand disease mechanisms (e.g., through inference on a parameter) ", Scientific.problem.being.solved))|> 
    mutate(How.is.uncertainty.in.calibration.outputs.reported. = ifelse(How.is.uncertainty.in.calibration.outputs.reported. == "Graphical: As a plot; e.g., a line with shaded areas indicating uncertainty intervals ", "Graphical: As a plot. E.g., a line with shaded areas indicating uncertainty intervals ", How.is.uncertainty.in.calibration.outputs.reported.))|> 
    mutate(How.is.uncertainty.in.calibration.outputs.reported. = ifelse(How.is.uncertainty.in.calibration.outputs.reported. == "Numerical: As uncertainty intervals around estimates or model outputs. e.g., 95% CI: 1.5 - 4.5.; Graphical: As a plot; e.g., a line with shaded areas indicating uncertainty intervals ", "Numerical: As uncertainty intervals around estimates or model outputs. e.g., 95% CI: 1.5 - 4.5.; Graphical: As a plot. E.g., a line with shaded areas indicating uncertainty intervals ", How.is.uncertainty.in.calibration.outputs.reported.))|> 
    mutate(Model.type = ifelse(Model.type == "Other: Compartmental", "Compartmental", Model.type))
```
...
# clean 'Other Type of data' Columns
```{r}
...
```
# Clean "Calibration size" Column
```{r}
# Format "Calibration size" Column
...
Names_alg <- c("History matching with emulation (HME); Approximate Bayesian Computation Markov Chain Monte Carlo (ABC-MCMC)",
               "Grey Wolf Optimizer (GWO); Markov Chain Monte Carlo",
               "Other; Approximate Bayesian Computation (ABC)",
               "Maximum likelihood estimation; Least squares estimation",
"History matching with emulation (HME)",
"Approximate Bayesian Computation Accept-Reject Random sampling (ABC-RS); Approximate Bayesian Computation Markov Chain Monte Carlo (ABC-MCMC)",
"Grey Wolf Optimizer (GWO); Markov Chain Monte Carlo",
"Manual (hand-tuning); Approximate Bayesian Computation Markov Chain Monte Carlo (ABC-MCMC)",
"Maximum Likelihood Estimation; Markov Chain Monte Carlo",
"Nelder-Mead algorithm; Incremental Mixture Importance Sampling (IMIS)",
"Other ; Manual (hand-tuning)",
"Broyden–Fletcher–Goldfarb–Shanno algorithm; Nelder–Mead algorithm; Sampling importance resampling (SIR)",
"Other",
"Incremental Mixture Importance Sampling (IMIS)")

Other_names_alg <- c(NA,
               NA,
               NA,
               NA,
               NA,
...
Nature_out <- c("Sample estimate (multiple parameter sets); Sample estimate (multiple parameter sets)",
                "Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Not reported ; Sample estimate (multiple parameter sets)", 
                "Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Sample estimate (multiple parameter sets)",
                "Not reported ; Sample estimate (multiple parameter sets)", 
                "Point estimate (single parameter / parameter set); Not reported ",
                "Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Sample estimate (multiple parameter sets); Not reported ",
                "Point estimate (single parameter / parameter set); Point estimate (single parameter / parameter set); Sample estimate (multiple parameter sets)",
                "Sample estimate (multiple parameter sets)",
                "Sample estimate (multiple parameter sets)")

out_size <- c("1000; 1000",
               "1; Not reported", 
               "Not reported; 1000", 
              "1; Not reported",
              "232-8857",
              ...
              "1; 1; 1000", 
              "1000 (Benin)",
              "690-2254 (country and model specific)")

clean_data_V5.2_dirty <- clean_data_V5.2 |>
  filter((is.na(What.is.the.size.of.the.calibration.output.V2) & What.is.the.size.of.the.calibration.output. != "Not reported"))|>
  mutate(What.is.the.size.of.the.calibration.output. = out_size)|>
  mutate(Name.of.calibration.algorithm = Names_alg)|>
  mutate(Nature.of.calibration.results = Nature_out)
...
ABC_SMC_names <- c("Approximate Bayesian Computation Sequential Monte Carlo scheme", 
                   ...
                   "Approximate Bayesian Computation with Sequential Monte Carlo Sampling",
                   "Approximate Bayesian Computation Sequential Monte Carlo Sampling (ABC-SMC) ",
...
clean_data_V11 <- clean_data_V10 |> 
  ...
  mutate(Calibration.algorithm.Family = ifelse(Calibration.algorithm.Family == "History matching with emulation (HME); Approximate Bayesian Computation Markov Chain Monte Carlo (ABC-MCMC)", "History matching with emulation (HME); Approximate Bayesian Computation (ABC)", Calibration.algorithm.Family))
...
write.xlsx(as.data.frame(clean_data_V11), file="Sheet S1.xlsx",
