...
Disease_type_Options <- c("HIV", "TB", "Malaria") 
df_0.2 <- data.frame(matrix(ncol = length(df_0.2_V0), nrow = length(Disease_type_Options)))
colnames(df_0.2) <- names(df_0.2_V0)
for(i in 1: length(Disease_type_Options)){
  df_0.2[i,] <- df_0.2_V0 |> 
  filter(grepl(Disease_type_Options[i], Options)) |> 
  summarise(Count=sum(Count),            
            Percentage= sum(Percentage))|> 
  mutate(Options =Disease_type_Options[i], .before = Count)|> 
  mutate(Field= "0.2 Disease type", .before = Options)
}
...
ordered_Name.of.calibration.algorithm <- clean_data |>
  filter(! Name.of.calibration.algorithm %in% c( "Not reported", "Other "))|>
  group_by(Name.of.calibration.algorithm)|>
  summarise(N=n())|>
  arrange(-N)|>
  pull(Name.of.calibration.algorithm)
df_C.2 <- clean_data |> 
    mutate(Name.of.calibration.algorithm = factor(Name.of.calibration.algorithm, levels= c(ordered_Name.of.calibration.algorithm, "Not reported", "Other " )))|> 
group_by(Name.of.calibration.algorithm) |>
  summarise(Count=n(), 
            Percentage= 100*round(Count/nrow(clean_data), 3))|> 
...
Objective1_df <- rbind(df_0.1, df_0.2, df_0.3, df_0.4, 
                       ...
                       df_C.1, df_C.2, df_C.3, df_C.4, df_C.5, df_C.6, 
